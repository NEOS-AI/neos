...
use clap::Parser;
use neos::config::{IndexerConfig, IndexerGraphConfig};
...
fn main() -> anyhow::Result<()> {
    ...
    neos::entrypoint::indexer::run(&IndexerConfig {
        output_path: path.to_str().unwrap().to_string(),
        limit_warc_files: None,
        skip_warc_files: None,
        warc_source: neos::config::WarcSource::Local(neos::config::LocalConfig {
            folder: ".".to_string(),
            names: vec![data_path
                .join("sample.warc.gz")
                .to_str()
        ...
        autocommit_after_num_inserts:
            neos::config::defaults::Indexing::autocommit_after_num_inserts(),
        dual_encoder: args
            .dual_encoder_path
            .map(|p| neos::config::IndexerDualEncoderConfig {
                model_path: p,
