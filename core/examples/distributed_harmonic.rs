...
use clap::Parser;
use neos::{distributed::member::ShardId, webgraph::Node, webpage::url_ext::UrlExt};
...
fn start_dht_thread(id: u64, host: SocketAddr, gossip: SocketAddr) {
    std::thread::spawn(move || {
        let config = neos::entrypoint::ampc::dht::Config {
            node_id: id,
            host,
            shard: ShardId::new(id),
            seed_node: None,
            gossip: Some(neos::config::GossipConfig {
        ...
        tokio::runtime::Builder::new_current_thread()
            ...
            .build()
            .unwrap()
            .block_on(neos::entrypoint::ampc::dht::run(config))
            .unwrap();
    });
}
...
fn start_worker_thread(graph_path: String, shard: ShardId, host: SocketAddr, gossip: SocketAddr) {
    std::thread::spawn(move || {
        let config = neos::config::HarmonicWorkerConfig {
            gossip: neos::config::GossipConfig {
                seed_nodes: Some(vec!["0.0.0.0:3001".parse().unwrap()]),
                addr: gossip,
            },
            shard,
        ...
        neos::entrypoint::ampc::harmonic_centrality::worker::run(config).unwrap();
    });
}
...
fn build_graphs_if_not_exist(warc_path: &str, graph_path: &str) -> anyhow::Result<()> {
    ...
    let warc = neos::warc::WarcFile::open(warc_path)?;
    ...
    let mut a =
        neos::webgraph::WebgraphWriter::new(a_path, Default::default(), Default::default(), None);

    let mut b =
        neos::webgraph::WebgraphWriter::new(b_path, Default::default(), Default::default(), None);

    for (i, record) in warc.records().flatten().enumerate() {
        let webpage = match neos::webpage::Html::parse_without_text(
...
fn main() -> anyhow::Result<()> {
    ...
    let config = neos::config::HarmonicCoordinatorConfig {
        gossip: neos::config::GossipConfig {
            seed_nodes: Some(vec!["0.0.0.0:3001".parse().unwrap()]),
    ...
    neos::entrypoint::ampc::harmonic_centrality::coordinator::run(config)?;
